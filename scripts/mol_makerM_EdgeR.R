...
# Load_count_files --------------------------------------------------------
setwd("/Users/elijahlowe/Desktop/tails/maker_snap_models/")


Mocu.3hpf <- read_tsv("SRR1197965_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocu_f3 = tot_counts, mocu_gene = target_id)
Mocu.3hpf #test to see if loaded properly
Mocu.4hpf <- read_tsv("SRR1197522_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocu_f4 = tot_counts, mocu_gene = target_id)
Mocu.6hpf <- read_tsv("SRR1197972_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocu_f6 = tot_counts, mocu_gene = target_id)
...
Mocc.3hpf   <- read_tsv("SRR1197985_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocc_f3 = tot_counts, mocc_gene = target_id)
Mocc.3.2hpf <- read_tsv("SRR1197986_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocc_f3.2 = tot_counts, mocc_gene = target_id)  
Mocc.4hpf   <- read_tsv("SRR1199464_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocc_f4 = tot_counts, mocc_gene = target_id)
Mocc.5hpf   <- read_tsv("SRR1199259_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocc_f5 = tot_counts, mocc_gene = target_id)
Mocc.6hpf   <- read_tsv("SRR1199268_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(Mocc_f6 = tot_counts, mocc_gene = target_id)
...
hyb_ocu.3hpf <- read_tsv("SRR1198337_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_ocu_f3 = tot_counts, mocu_gene = target_id)
hyb_ocu.4hpf <- read_tsv("SRR1198321_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_ocu_f4 = tot_counts, mocu_gene = target_id)
hyb_ocu.6hpf <- read_tsv("SRR1198346_mocu_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_ocu_f6 = tot_counts, mocu_gene = target_id)
...
hyb_occ.3hpf <- read_tsv("SRR1198337_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_occ_f3 = tot_counts, mocc_gene = target_id)
hyb_occ.4hpf <- read_tsv("SRR1198321_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_occ_f4 = tot_counts, mocc_gene = target_id)
hyb_occ.6hpf <- read_tsv("SRR1198346_mocc_snap_maker_express2.xprs") %>% 
  dplyr::rename(hyb_occ_f6 = tot_counts, mocc_gene = target_id)
...
recip_table <- read_tsv('maker_synteny.poff', col_types = list(col_double(),col_double(),col_double(),col_character(),col_character(),col_character()), col_names = c("# Species","Genes", "Alg.-Conn.", "KH", "mocc_gene","mocu_gene")) %>% 
  filter(`# Species` >= 2, !grepl('\\*|,', mocc_gene),!grepl('\\*|,', mocu_gene))
# %>% 
#   mutate(mocu_gene = strsplit(as.character(mocu_gene), ",")) %>% 
#   unnest(mocu_gene) %>% 
#   mutate(mocc_gene = strsplit(as.character(mocc_gene), ",")) %>% 
#   unnest(mocc_gene)

recips <- recip_table %>% 
  select(-KH, -`# Species`, -Genes, -`Alg.-Conn.`)

all.data <- inner_join(Mocu.data, recips) %>% 
...
d3 <- estimateDisp(edger.data,design)
...
plotSmear(et.mocu6vmocc6, ylim=c(-10,10), de.tags=detags, main="M. oculata vs M. occulta 4hpf")
...
mocc.4v6.down <- rownames_to_column(topTags(et.mocc.4v6, n=nrow(et.mocc.4v6$table))$table, "gene") %>%
  as_tibble() %>%
  filter(logFC < -1.5, FDR < 0.05) %>%
  # inner_join(recips, by = c("gene" = "mocu_gene")) %>%
  # mutate(gene = mocc_gene) %>% 
  # select(-mocc_gene) %>% 
# mocc.4v6.down %>%
...
topTags(et.mocu.3v6, n=nrow(et.mocu.3v6$table))$table %>% 
  rownames_to_column(var = "gene") %>% 
  ...
  rename(logFC.mocu_tb_gt = logFC.x, logCPM.mocu_tb_gt = logCPM.x, PValue.mocu_tb_gt = PValue.x, FDR.mocu_tb_gt = FDR.x,
         ...
         logFC.hyb_gt = logFC.y.y, logCPM.hyb_gt = logCPM.y.y, PValue.hyb_gt = PValue.y.y, FDR.hyb_gt = FDR.y.y,
         logFC.hyb_tb = logFC, logCPM.hyb_tb = logCPM, PValue.hyb_tb = PValue, FDR.hyb_tb = FDR) %>% 
  write_csv("3v6_Mol_GG_edgeR_maker_results.csv")
...
sp_groups <- c('M. oculata neur v gast up' = 'mocu', 'M. oculata neur v gast down' = 'mocu', 'M. occulta neur v gast up' = 'mocc', 'M. occulta neur v gast down' = 'mocc', 'M. hybrid neur v gast up' = 'hyb', 'M. hybrid neur v gast down' = 'hyb',
               'M. oculata tail v neur up' = 'mocu', 'M. oculata tail v neur down' = 'mocu', 'M. occulta tail v neur up' = 'mocc', 'M. occulta tail v neur down' = 'mocc', 'M. hybrid tail v neur up' = 'hyb', 'M. hybrid tail v neur down' = 'hyb',
               'M. oculata tail v gast up' = 'mocu', 'M. oculata tail v gast down' = 'mocu', 'M. occulta tail v gast up' = 'mocc', 'M. occulta tail v gast down' = 'mocc', 'M. hybrid tail v gast up' = 'hyb', 'M. hybrid tail v gast down' = 'hyb')
pdf('molgula_de_upsetplot.pdf')
upset(fromList(listInput), order.by = "freq", nsets = 18,left_annotation = rowAnnotation(group = sp_groups[set_name(listInput)], show_annotation_name = FALSE))
dev.off()
?upset
